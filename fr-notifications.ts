...
function buildFriendPinnedEmbed(payload: Record<string, unknown>): EmbedBuilder {
  ...
  return new EmbedBuilder()
    ...
    .setDescription(
      [
        isPinned ? `A user was added to your pinned list.` : `A user was removed from your pinned list.`,
        "",
  ...
  return new EmbedBuilder()
    ...
    .setDescription(
      [
        ...
        success && count != null ? `**Removed:** ${count} friend${count !== 1 ? "s" : ""}` : null,
        !success && errorMsg     ? `**Error:** ${errorMsg}` : null,
...
function buildSystemEmbed(payload: Record<string, unknown>): EmbedBuilder {
  return new EmbedBuilder()
    ...
    .setDescription((payload.message as string | undefined) ?? "A system event occurred.")
    .setColor(0x9b59b6)
    .setTimestamp();
}
...
function isEventEnabled(eventType: NotificationEventType, prefs: FRNotificationPrefs): boolean {
  switch (eventType) {
    case "friend_removed":              return prefs.friendRemoved;
    case "friend_pinned":
    case "friend_unpinned":             return prefs.friendPinned;
    case "scheduled_removal_ran":
    case "scheduled_removal_failed":    return prefs.scheduledRemovals;
    case "system":                      return prefs.systemEvents;
 ...
 * Looks up the Discord link from KV — survives bot restarts.
 */
export async function sendFRNotification(
  ...
  const discordUser = await findDiscordUserByRobloxId(robloxUserId);
  if (!discordUser) {
    console.log(`[FR NOTIF] No Discord link found for Roblox ID ${robloxUserId}`);
    return;
  }

  if (!isEventEnabled(eventType, discordUser.notificationPrefs)) {
    console.log(`[FR NOTIF] Event ${eventType} disabled for ${discordUser.discordId}`);
    return;
  }

  const notif = await createNotification(discordUser.discordId, robloxUserId, eventType, payload);

  try {
    const user = await client.users.fetch(discordUser.discordId).catch(() => null);
    if (!user) {
      console.warn(`[FR NOTIF] Could not fetch Discord user ${discordUser.discordId}`);
      return;
    }

    const dmChannel = await user.createDM().catch(() => null);
    if (!dmChannel) {
      console.warn(`[FR NOTIF] Could not open DM with ${discordUser.discordId}`);
      return;
    }
    ...
    await markNotificationReceived(robloxUserId, notif.id);
    console.log(`[FR NOTIF] ✅ Sent ${eventType} to ${discordUser.discordId}`);
  } catch (err) {
    console.error(`[FR NOTIF] ❌ Failed to deliver to ${discordUser.discordId}:`, err);
  }
}
